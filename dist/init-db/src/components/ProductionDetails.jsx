...
exports.default = ProductionDetails;
const react_1 = __importDefault(require("react"));
const navigation_1 = require("next/navigation");
const outline_1 = require("@heroicons/react/24/outline");
const image_1 = __importDefault(require("next/image"));
function ProductionDetails({ title, icon, item, loading, error, additionalFields = [], }) {
        ...
        <div className="bg-white rounded-lg shadow">
          ...
          <div className="px-6 py-4 border-b border-gray-200">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <button type="button" onClick={() => router.push(`/productions/${item.segment}/${item.id}/edit`)} className="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-primary rounded-md hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
                  <outline_1.PencilIcon className="w-4 h-4 mr-2"/>
                  Edit
                </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div className="md:col-span-1">
                <div className="relative aspect-square rounded-lg overflow-hidden bg-gray-100">
                  {item.image ? (<image_1.default src={item.image} alt={item.name} fill className="object-cover"/>) : (<div className="absolute inset-0 flex items-center justify-center text-gray-400">
                      {icon}
                    </div>)}
                </div>
              </div>
              ...
              <div className="md:col-span-2 space-y-6">
                ...
                {/* Additional Fields */}
                {additionalFields.map((field) => (<div key={field.name}>
                ...
                {item.dependencies && item.dependencies.length > 0 && (<div>
